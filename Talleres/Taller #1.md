...
2.  **Creación de Consultas de Agregación**:
   ...
   - Ejecutes las siguientes consultas para visualizar los estudiantes y profesores:
```sql
...
![taller1-imagen.png](../assets%2Ftaller1-imagen.png)

4. Crear un nuevo servidor en pgAdmin para conectarse al proyecto creado en Neon, para esto:
   a)  En Neon visualice el connection string que aparece en la pantalla después de crear el proyecto. (El connection string es algo como esto `postgresql://neondb_owner:XXXXXXXXXX@ep-tight-brook-a5xgpz7i.us-east-2.aws.neon.tech/neondb?sslmode=require`)
   b) Acceda al siguiente link para aprender la estructura del connection string https://www.postgresql.org/docs/current/libpq-connect.html#LIBPQ-CONNSTRING
   c) En pgAdmin haga click derecho sobre Servers y seleccione **Register → Server.**
   d)  Asigne un nombre al nuevo servidor. Ejemplo: `Neon BD 2024 - 2`.
   e) Vaya al tab de **Connection** y llene los campos con base en el literal (b). Seleccione **Save password?** para que la próxima vez no tenga que ingresar el password de nuevo.
   f) Finalmente haga click en **Save,** si todo estuvo correcto, el nuevo servidor debe aparecer en la lista de servidores.
   g) Explore la opción de Dashboard y verifique todos los parámetros.
5. Repetir los **pasos 1-5** de este taller en Neon. Use este archivo de respaldo:
...
6. Explore la página web de Neon y tome nota de cualquier cosa que le llame la atención. Por ejemplo
   a) Neon le genera unos snippets de código para realizar la conexión usando distintos lenguajes de programación.
   b) Neon cuenta con su propio SQL Editor que le permite ejecutar consultas.
   c) Neon le permite monitorear métricas de su bases de datos relacionadas con CPU, memoria, conexiones, etc.
---
